...
public class SetStringRecord implements LogRecord {
    ...
    */
   public SetStringRecord(int txnum, Block blk, int offset, String oldValue, String newValue) {
      this.txnum = txnum;
      this.blk = blk;
      this.offset = offset;
   ...
   public SetStringRecord(BasicLogRecord rec) {
      ...
      offset = rec.nextInt();
      oldValue = rec.nextString();
      newValue = rec.nextString();
   }
   ...
   public void undo(int txnum) {
      ...
      buff.setString(offset, oldValue, txnum, -1);
      buffMgr.unpin(buff);
   }
   ...
   public void redo(int txnum) {
      BufferMgr buffMgr = SimpleDB.bufferMgr();
      Buffer buff = buffMgr.pin(blk);
      buff.setString(offset, newValue, txnum, -1);
      buffMgr.unpin(buff);
   }

...
public class SetIntRecord implements LogRecord {
   private int txnum, offset, oldValue, newValue;
   private Block blk;
    ...
    */
   public SetIntRecord(int txnum, Block blk, int offset, int oldValue, int newValue) {
      this.txnum = txnum;
      this.blk = blk;
      this.offset = offset;
   ...
   public SetIntRecord(BasicLogRecord rec) {
      ...
      offset = rec.nextInt();
      oldValue = rec.nextInt();
      newValue = rec.nextInt();
   }
   ...
   public int writeToLog() {
      Object[] rec = new Object[] {SETINT, txnum, blk.fileName(),
         blk.number(), offset, oldValue, newValue};
   ...
   public void undo(int txnum) {
      ...
      buff.setInt(offset, oldValue, txnum, -1);
      buffMgr.unpin(buff);
   }
   ...
   public void redo(int txnum) {
	      BufferMgr buffMgr = SimpleDB.bufferMgr();
	      Buffer buff = buffMgr.pin(blk);
	      buff.setInt(offset, newValue, txnum, -1);
	      buffMgr.unpin(buff);
	   }
